...
인수 테스트 208개 통과 (본체) + 26개 (VS Code 확장).

```
src/store.py       스키마, 검증, 파일 잠금, 원자적 교체
...
tests/             T1~T14 (기능), W1~W7 (이식성), 훅 42개, R1~R10 (견고성),
                   K1~K14 (WIP 상한), D1~D7 (세션 복원), DR1~DR9 (자가진단)
...
| 시점 | 하는 일 |
|---|---|
| 세션 시작·압축·`/clear`·fork | 현재 경로와 복귀지점, 분기규칙을 되살려 준다 |
| 프롬프트마다 | 현재 경로와 분기규칙을 한 줄 주입. WIP 상한에 닿았거나 오래 열린 노드가 있으면 알린다 |
| 툴 실행 후 | 에러·경고·`TODO` 를 보류함에 **자동으로 담고** 한 줄 알린다 |
| 세션 종료 | 열린 노드·보류함·게이트 횟수 요약 |
...
### 손상·극단 입력에서 무엇이 보장되는가

`state.json` 은 `wn.py` 만 쓴다. 그래도 사람이 열어 고치고, 디스크가 멈추고,
파일이 디렉터리가 되기도 한다. 계약은 셋이다.

1. **표시 경로는 어떤 입력에도 조용하다.** 상태줄과 훅은 출력도 오류도 내지 않는다.
2. **CLI 는 파이썬 스택을 사람에게 보여주지 않는다.** 손상은 손상(exit 4)이라고 말한다.
...
`/wn-doctor` 를 치면 된다. 설정을 읽는 게 아니라 훅을 임시 프로젝트에서 **실제로 구동**해서
종료코드·출력 모양·지연을 보고, 마켓플레이스 사본과 설치본이 같은지, 두 표지판(터미널·상태바)이
같은 문자열을 내는지까지 대조한다. 사용자의 작업 상태는 바꾸지 않는다.

가장 중요한 줄은 `주입 기록` 이다. "등록은 됐는데 한 번도 안 불렸다" 는 여기서만 보인다.
